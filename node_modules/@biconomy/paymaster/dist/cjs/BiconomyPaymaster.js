...
class BiconomyPaymaster {
    ...
    async prepareUserOperation(userOp) {
        ...
        try {
            const keys1 = ["nonce", "maxFeePerGas", "maxPriorityFeePerGas"];
            for (const key of keys1) {
                if (userOperation[key] && userOperation[key] !== "0x") {
                    userOperation[key] = ("0x" + BigInt(userOp[key]).toString(16));
                }
            }
            const keys2 = ["callGasLimit", "verificationGasLimit", "preVerificationGas"];
    ...
    async getPaymasterFeeQuotesOrData(userOp, paymasterServiceData) {
        ...
        try {
            ...
            if (response && response.result) {
                ...
                else if (response.result.mode == Types_js_1.PaymasterMode.SPONSORED) {
            ...
            if (!this.paymasterConfig.strictMode &&
                ...
                common_1.Logger.warn(`Strict mode is ${this.paymasterConfig.strictMode}. sending paymasterAndData 0x`);
                return {
                    paymasterAndData: "0x",
    ...
    async getPaymasterAndData(userOp, paymasterServiceData) {
        ...
        if (mode === Types_js_1.PaymasterMode.ERC20) {
            if (!paymasterServiceData?.feeTokenAddress && paymasterServiceData?.feeTokenAddress === Constants_js_1.ADDRESS_ZERO) {
